...
export default function BatchDetailPage({ params }: PageProps) {
    ...
    const { data: batchData } = useQuery({
        ...
        refetchInterval: (query) => {
            ...
            return batch.status === 'pending' || batch.status === 'processing' ? 3000 : false;
        },
    });
                ...
                <>
                    <div className={styles.pageHeader}>
                        <h1 className={styles.pageTitle}>
                            Batch <span className={styles.batchId}>{batch.id.slice(0, 8)}</span>
                        </h1>
                        <div className={styles.meta}>
                            <StatusBadge status={batch.status} />
                            <span className={styles.date}>{formatDate(batch.created_at)}</span>
                        </div>
                    </div>
                    ...
                    <div className={styles.stats}>
                        <span>
                            Total: <strong className={styles.statValue}>{batch.total_items}</strong>
                        </span>
                        <span>
                            Complete:{' '}
                            <strong className={styles.statValue}>{batch.completed_items}</strong>
                        </span>
                        <span>
                            Failed:{' '}
